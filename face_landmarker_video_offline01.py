...
class FaceLandmarkerVideoOffline:
    def __init__(self):
        ...
        self.output_video_name = "processed_output01.mp4"  # 最终输出视频文件名
        
        # 处理配置参数
        self.enable_alignment_detection = True  # 是否启用前60帧对齐检测
        ...
        # 默认处理参数
        self.enable_pixel_warp = False  # 像素级变形（初始关闭，对齐后自动开启）
        self.enable_face_only_mode = False  # 面部专用模式（初始关闭，对齐后自动开启）
        self.enable_lambert_material = True  # Lambert材质渲染
        self.enable_wireframe = False  # 黑色线框显示
        ...
        # 手动设置相机参数 (无需真实校准)
        self.manual_fx = self.video_width
        self.manual_fy = self.video_height
        self.manual_cx = self.video_width / 2
        self.manual_cy = self.video_height / 2
        ...
        print(f"对齐帧数: {self.alignment_frames}")
        print(f"自动启用像素变形: {'是' if self.auto_enable_pixel_warp else '否'}")
        print(f"自动启用面部专用模式: {'是' if self.auto_enable_face_only_mode else '否'}")
    ...
    def update_video_aspect_ratio(self, video_width, video_height):
        ...
        self.manual_cx = video_width / 2.0   # 主点X坐标（图像中心）
        self.manual_cy = video_height / 2.0  # 主点Y坐标（图像中心）
